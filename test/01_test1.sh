...
CLUBFACTORYSOL=`grep ^CLUBFACTORYSOL= settings.txt | sed "s/^.*=//"`
CLUBFACTORYJS=`grep ^CLUBFACTORYJS= settings.txt | sed "s/^.*=//"`
...
printf "CLUBFACTORYSOL     = '$CLUBFACTORYSOL'\n" | tee -a $TEST1OUTPUT
...
DIFFS1=`diff $SOURCEDIR/$CLUBFACTORYSOL $CLUBFACTORYSOL`
...
// -----------------------------------------------------------------------------
var deployMembersLibMessage = "Deploy Members Library";
// -----------------------------------------------------------------------------
console.log("RESULT: ----- " + deployMembersLibMessage + " -----");
...
var membersLibContract = membersLibContract.new({from: contractOwnerAccount, data: membersLibBin, gas: 6000000, gasPrice: defaultGasPrice},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
        ...
        addAccount(membersLibAddress, "Members Library");
